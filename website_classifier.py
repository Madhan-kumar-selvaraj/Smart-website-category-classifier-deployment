        ...
        if len(strip_data) > 30:
            ...
            if result_percent > 30 :
                if len(strip_data)<500:
                    return (str(target_dict[y_predict[0]] ), "Note: Classification result may be inaccurate due to minimal content in the website and it's accuracy is " + str(result_percent) + " %" , "You can add you own category for your website. If the name of the category peresent in the below list use that name. Or else create your own in this format https://website_classifier.herokuapp.com/(url)?category=(category). For exmaple: http://127.0.0.1:8000/https:**www.mdpi.com*journal*agriculture?category=agriculture", category_url_list, words_most_frequent)
                else:
                    return (target_dict[y_predict[0]], "Accuracy of the classification is " + str(result_percent) + " %" , "You can add you own category for your website. If the name of the category peresent in the below list use that name. Or else create your own in this format http://127.0.0.1:8000/(url)?category=(category). For exmaple: https://website_classifier.herokuapp.comhttps:**www.mdpi.com*journal*agriculture?category=agriculture", category_url_list , words_most_frequent)
            else:
